...
struct ShoppingListNetwork {
    ...
    func getShoppingList() async throws -> ShoppingListResponse {
        let response = try await ZapNetwork.service().get("/shopping_lists/current")
    ...
    func getLeftovers() async throws -> LeftoverList {
        let response = try await ZapNetwork.service().get("/shopping_lists/leftover")
    ...
    func getPastLists() async throws -> ShoppingListCollectionResponse {
        let response = try await ZapNetwork.service().get("/shopping_lists/past_lists")
